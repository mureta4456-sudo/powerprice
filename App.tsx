...
import {
  ...
  Info, ChevronRight, HelpCircle, BookOpen, Download, ArrowLeft, Calendar, Clock, Tag
} from "lucide-react";
import {
...
// All 24 EU official languages — matches keys in src/i18n.ts
const LANGUAGES = [
...
/**
 * Tiny Markdown renderer — supports ##/### headings, **bold**,
 * unordered (-) and ordered (1.) lists, [links](url) and paragraphs.
 */
...
// ─────────────────────────────────────────── PRICES VIEW (home) ───
function HomeView({ goBlog }: { goBlog: () => void }) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={goBlog} className="text-xs font-bold text-emerald-600 hover:text-emerald-700 transition-colors">
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-12">
      ...
      {route.view === "home" && <HomeView goBlog={() => navigate({ view: "blog" })} />}
